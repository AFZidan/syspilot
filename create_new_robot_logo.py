...
def create_robot_logo():
    ...
    draw.ellipse([
        ...
        center_x + head_radius, center_y + head_radius
    ], fill=primary_color, outline=dark_color, width=8)
    
    # Robot antenna/top element
    antenna_height = size // 8
    antenna_width = size // 20
    draw.rectangle([
        center_x - antenna_width//2, center_y - head_radius - antenna_height,
        center_x + antenna_width//2, center_y - head_radius
    ], fill=dark_color)
    
    # Antenna tip
    tip_radius = size // 30
    draw.ellipse([
        center_x - tip_radius, center_y - head_radius - antenna_height - tip_radius,
        center_x + tip_radius, center_y - head_radius - antenna_height + tip_radius
    ], fill=accent_color)
    ...
    # Left eye
    draw.ellipse([
        center_x - eye_offset - eye_radius, center_y - size//12 - eye_radius,
        center_x - eye_offset + eye_radius, center_y - size//12 + eye_radius
    ], fill=light_color, outline=dark_color, width=4)
    ...
    draw.ellipse([
        center_x - eye_offset - pupil_radius, center_y - size//12 - pupil_radius,
        center_x - eye_offset + pupil_radius, center_y - size//12 + pupil_radius
    ], fill=accent_color)
    ...
    # Left panel
    draw.rectangle([
        center_x - panel_offset - panel_width, center_y - panel_height//2,
        center_x - panel_offset, center_y + panel_height//2
    ], fill=secondary_color, outline=dark_color, width=4)
    ...
    for panel_x in [center_x - panel_offset - panel_width//2, center_x + panel_offset + panel_width//2]:
        for detail_y in [center_y - panel_height//4, center_y + panel_height//4]:
            draw.ellipse([
                panel_x - detail_radius, detail_y - detail_radius,
                panel_x + detail_radius, detail_y + detail_radius
            ], fill=accent_color)
    ...
    neck_height = size // 12
    draw.rectangle([
        center_x - neck_width//2, center_y + head_radius,
        center_x + neck_width//2, center_y + head_radius + neck_height
    ], fill=dark_color)
...
def create_banner_version(logo_img, width=800, height=200):
    ...
    shadow_offset = 2
    draw.text((text_x + shadow_offset, text_y + shadow_offset), text, font=font, fill=(44, 62, 80, 128))
