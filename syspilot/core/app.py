...
from PyQt5.QtWidgets import (
    ...
    QCheckBox, QComboBox, QSpinBox, QTimeEdit, QFormLayout,
...
class ScheduleDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        self.frequency_combo.addItems(["monday", "tuesday", "wednesday", "thursday", 
...
class SysPilotApp:
    ...
    def __init__(self, config_path=None):
        ...
        self.scheduling_service = SchedulingService(self.config, self.cleanup_service) if SchedulingService else None
    ...
    def run(self):
        ...
        try:
            ...
            possible_paths = [
                Path(__file__).parent.parent / "assets" / "syspilot_icon.png",  # Development
                Path("/usr/local/lib/syspilot/syspilot/assets/syspilot_icon.png"),  # System install
                Path("/usr/share/syspilot/assets/syspilot_icon.png"),  # Alternative system location
            ]
    ...
    def create_monitoring_tab(self):
        ...
        # CPU temperature
        self.monitoring_widgets['cpu_temp_label'] = QLabel("CPU Temperature: Loading...")
    ...
    def update_monitoring_data(self, data):
        ...
        if 'cpu_temperature' in data and data['cpu_temperature'] is not None:
            ...
            if data['cpu_temperature'] > 80:
                self.monitoring_widgets['cpu_temp_label'].setStyleSheet("color: red; font-weight: bold;")
            elif data['cpu_temperature'] > 70:
                self.monitoring_widgets['cpu_temp_label'].setStyleSheet("color: orange; font-weight: bold;")
            elif data['cpu_temperature'] > 60:
                self.monitoring_widgets['cpu_temp_label'].setStyleSheet("color: #FFA500;")  # Dark orange
            else:
    ...
    def update_settings_ui(self):
        ...
        try:
            ...
            if self.autostart_service:
                ...
                if status['enabled']:
                    self.autostart_status_label.setText("Auto-start is currently ENABLED")
                    self.enable_autostart_btn.setEnabled(False)
                    self.disable_autostart_btn.setEnabled(True)
                else:
                    self.autostart_status_label.setText("Auto-start is currently DISABLED")
                    self.enable_autostart_btn.setEnabled(True)
                    self.disable_autostart_btn.setEnabled(False)
            else:
                self.autostart_status_label.setText("Auto-start feature not available (missing dependencies)")
                self.enable_autostart_btn.setEnabled(False)
                self.disable_autostart_btn.setEnabled(False)
    ...
    def update_schedules_list(self):
        ...
        try:
            ...
            if not self.scheduling_service:
                self.schedules_list.addItem("Scheduling feature not available (missing dependencies)")
                return
            ...
            for schedule_id, schedule_data in schedules.items():
                status = "Enabled" if schedule_data.get('enabled', False) else "Disabled"
                schedule_type = schedule_data.get('type', 'unknown')
                time_str = schedule_data.get('time', 'N/A')
                
                item_text = f"{schedule_id} - {schedule_type.title()} at {time_str} ({status})"
