    ...
    def _schedule_cleanup_tasks(self):
        ...
        try:
            if self.config.get('daemon', 'auto_cleanup', False):
                cleanup_schedule = self.config.get('daemon', 'cleanup_schedule', '0 2 * * *')
    ...
    def get_daemon_status(self) -> dict:
        ...
        try:
            status = {
                ...
                'monitoring_enabled': self.config.get('daemon', 'monitoring_enabled', True),
                'auto_cleanup_enabled': self.config.get('daemon', 'auto_cleanup', False),
                'cleanup_schedule': self.config.get('daemon', 'cleanup_schedule', '0 2 * * *'),
