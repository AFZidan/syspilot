...
class CleanupService:
    ...
    def full_cleanup(self, 
        ...
        try:
            ...
            self.logger.info(f"Space freed: {self._format_bytes(self.stats['space_freed'])}")
    ...
    def _clean_log_files(self):
        ...
        for pattern in log_patterns:
            try:
                ...
                self.logger.error(f"Error cleaning log files with pattern {pattern}: {e}")
    ...
    def _clean_package_cache(self):
        ...
        try:
            ...
            subprocess.run(['sudo', 'apt-get', 'clean'], check=True, capture_output=True)
            
            # Clean apt auto-remove
            subprocess.run(['sudo', 'apt-get', 'autoremove', '-y'], check=True, capture_output=True)
            
            # Clean apt auto-clean
            subprocess.run(['sudo', 'apt-get', 'autoclean'], check=True, capture_output=True)
    ...
    def _clean_directory(self, directory: str, max_age_days: int, exclude_patterns: List[str]):
        """
        ...
        try:
            ...
            for root, dirs, files in os.walk(directory):
                ...
                for file in files:
                    ...
                    # Check file age
                    if max_age_days > 0 and not self._is_file_old(file_path, max_age_days):
                        continue
    ...
    def _update_package_database(self):
        ...
        try:
            ...
            self.logger.debug("Could not update package database (updatedb not available)")
        except Exception as e:
        ...
        try:
            ...
            for temp_dir in temp_dirs:
                if os.path.exists(temp_dir):
                    files = self._get_files_to_clean(temp_dir, max_age_days, exclude_patterns)
            ...
            for cache_dir in cache_dirs:
                if os.path.exists(cache_dir):
                    files = self._get_files_to_clean(cache_dir, max_age_days, exclude_patterns)
    ...
    def _get_files_to_clean(self, directory: str, max_age_days: int, exclude_patterns: List[str]) -> List[str]:
