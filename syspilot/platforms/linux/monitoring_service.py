            ...
            if hasattr(psutil, 'sensors_temperatures'):
                ...
                for source in temp_sources:
                    if source in temps and temps[source]:
                        ...
                        if hasattr(temp_sensor, 'current'):
                            self.logger.debug(f"CPU temperature from {source}: {temp_sensor.current}°C")
                ...
                for source_name, sensors in temps.items():
                    if sensors and len(sensors) > 0:
                        ...
                        if hasattr(temp_sensor, 'current'):
                            self.logger.debug(f"CPU temperature from {source_name}: {temp_sensor.current}°C")
                            return round(temp_sensor.current, 1)
                    ...
                    if os.path.exists(zone_path):
                        with open(zone_path, 'r') as f:
                            ...
                            if 0 < temp_celsius < 150:  # Reasonable temperature range
                                self.logger.debug(f"CPU temperature from {zone_path}: {temp_celsius}°C")
    ...
    def _get_network_io(self) -> Dict:
        ...
        try:
            ...
            if self.prev_network_io and self.prev_network_time:
                ...
                if time_diff > 0:
                    bytes_sent_rate = (network_io.bytes_sent - self.prev_network_io.bytes_sent) / time_diff / 1024  # KB/s
                    bytes_recv_rate = (network_io.bytes_recv - self.prev_network_io.bytes_recv) / time_diff / 1024  # KB/s
    ...
    def _get_system_load(self) -> Dict:
        ...
        try:
            ...
            return {
                ...
                'cpu_count': cpu_count,
                'load_1min_percent': (load_avg[0] / cpu_count) * 100 if cpu_count > 0 else 0,
                'load_5min_percent': (load_avg[1] / cpu_count) * 100 if cpu_count > 0 else 0,
                'load_15min_percent': (load_avg[2] / cpu_count) * 100 if cpu_count > 0 else 0
            }
        except Exception as e:
