...
class SystemInfoService:
                ...
                with open('/proc/cpuinfo', 'r') as f:
                    for line in f:
                        if line.startswith('cpu MHz'):
                            cpu_info['frequency'] = f"{float(line.split(':')[1].strip()):.0f} MHz"
                            break
            except Exception:
                pass
            ...
            with open('/proc/mounts', 'r') as f:
                for line in f:
                    ...
                    if len(parts) >= 3:
                        ...
                        if fstype in ['ext4', 'ext3', 'ext2', 'btrfs', 'xfs', 'ntfs', 'fat32', 'vfat']:
                            try:
                ...
                if result.returncode == 0:
                    ...
                    for line in lines:
                        if 'VGA compatible controller' in line:
                            graphics_info['gpu'] = line.split(': ')[1] if ': ' in line else line
                            break
            except Exception:
                pass
    ...
    def get_disk_usage_by_directory(self, directory: str = '/') -> List[Dict]:
        ...
        try:
            ...
            result = subprocess.run(['du', '-h', '--max-depth=1', directory], 
                                  capture_output=True, text=True)
    ...
    def get_largest_files(self, directory: str = '/', limit: int = 10) -> List[Dict]:
        ...
        try:
            ...
            result = subprocess.run(['find', directory, '-type', 'f', '-exec', 'ls', '-la', '{}', '+'], 
                                  capture_output=True, text=True)
