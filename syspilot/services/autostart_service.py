...
class AutoStartService:
    ...
    def __init__(self):
        ...
        self.desktop_file_path = Path.home() / ".config" / "autostart" / "syspilot.desktop"
        self.systemd_service_path = Path.home() / ".config" / "systemd" / "user" / "syspilot.service"
    ...
    def disable_autostart(self) -> bool:
        ...
        try:
            ...
            if self.systemd_service_path.exists():
                ...
                subprocess.run(["systemctl", "--user", "stop", "syspilot.service"], 
                             capture_output=True, check=False)
                subprocess.run(["systemctl", "--user", "disable", "syspilot.service"], 
                             capture_output=True, check=False)
...
"""
            ...
            self.logger.info(f"Created desktop auto-start entry: {self.desktop_file_path}")
            return True
...
"""
            ...
            subprocess.run(["systemctl", "--user", "enable", "syspilot.service"], 
                         capture_output=True, check=True)
            
            self.logger.info(f"Created systemd auto-start service: {self.systemd_service_path}")
            return True
    ...
    def _get_syspilot_path(self) -> Optional[str]:
        ...
        try:
            ...
            result = subprocess.run(["which", "syspilot"], 
