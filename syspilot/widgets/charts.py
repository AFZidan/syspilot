...
class ThermometerWidget(BaseChartWidget):
    ...
    def update_data(self, temperature):
        ...
        tube_x = [-tube_width/2, tube_width/2, tube_width/2, -tube_width/2, -tube_width/2]
        tube_y = [bulb_radius, bulb_radius, bulb_radius + tube_height, bulb_radius + tube_height, bulb_radius]
        ...
        ax.plot(np.cos(np.linspace(0, 2*np.pi, 100)) * bulb_radius, 
                np.sin(np.linspace(0, 2*np.pi, 100)) * bulb_radius, 'k-', linewidth=2)
        ...
        if mercury_height > 0:
            mercury_tube_x = [-tube_width/2 * 0.7, tube_width/2 * 0.7, tube_width/2 * 0.7, -tube_width/2 * 0.7]
            mercury_tube_y = [bulb_radius, bulb_radius, bulb_radius + mercury_height, bulb_radius + mercury_height]
        ...
        for i in range(0, 101, 10):
            ...
            # Right side markings
            ax.plot([tube_width/2, tube_width/2 + 0.03], [y_pos, y_pos], 'k-', linewidth=1)
            if i % 20 == 0:  # Major markings
                ax.text(tube_width/2 + 0.06, y_pos, f'{i}°', ha='left', va='center', fontsize=8)
        
        # Add temperature value display
        ax.text(0, -0.35, f'{temperature:.1f}°C', ha='center', va='center', 
                fontsize=16, fontweight='bold', color=mercury_color)
        ...
        ax.text(0, -0.45, status, ha='center', va='center', 
                fontsize=10, style='italic', color=mercury_color)
...
class SystemMonitoringWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        self.cpu_thermometer = ThermometerWidget("CPU Temperature", min_temp=0, max_temp=100)
    ...
    def update_data(self, system_data):
        ...
        try:
            ...
            if network_io:
                ...
                max_speed = max(bytes_sent, bytes_recv, 100)  # Minimum scale of 100 KB/s
